"""
FiscoSmart — Página 5: Taxas Municipais e COSIP v2
FIX: usa cosip_mensal.csv (144 linhas) em vez de cosip.csv (648k linhas)
"""
...
def load():
    ...
    # FIX: usa arquivo pré-agregado — 144 linhas em vez de 648k
    cosip_m    = pd.read_csv(f"{DATA_RAW}/cosip_mensal.csv")
    ...
    return alv, cosip_m, lixo, cm, contr


alv, cosip_m, lixo, cm, contribs = load()
...
cosip_rec  = cosip_m["valor_pago"].sum()
cosip_lanc = cosip_m["valor_lancado"].sum()
...
with col1:
    ...
    alv_porte = (
        ...
        .groupby(["porte","ano"])
        .agg(lanc=("valor_lancado","sum"), pago=("valor_pago","sum"))
        .reset_index()
    )
    alv_porte["tx_inad"] = (1 - alv_porte["pago"] / alv_porte["lanc"].clip(lower=1)) * 100
    fig_alv = px.bar(
        alv_porte, x="porte", y="tx_inad", color="ano", barmode="group",
...
with col2:
    ...
    # Agrega por faixa (soma todos os meses)
    cosip_faixa = cosip_m.groupby("faixa_consumo").agg(
        arrecadado=("valor_pago","sum"),
        lancado=("valor_lancado","sum"),
    ).reset_index()
    cosip_faixa["tx_adim"] = cosip_faixa["arrecadado"] / cosip_faixa["lancado"] * 100
...
cosip_ev = (
    cosip_m.groupby("competencia")
    .agg(lancado=("valor_lancado","sum"), arrecadado=("valor_pago","sum"))
    .reset_index()
    .sort_values("competencia")
)
cosip_ev["gap"] = cosip_ev["lancado"] - cosip_ev["arrecadado"]
fig_cm = go.Figure()
fig_cm.add_scatter(x=cosip_ev["competencia"], y=cosip_ev["lancado"],
                   name="Lançado", line=dict(color="#c8860a", dash="dash"))
fig_cm.add_scatter(x=cosip_ev["competencia"], y=cosip_ev["arrecadado"],
...
for obra_id in cm["id_obra"].unique():
    ...
    with st.expander(f"🏗️ Obra {obra_id}: {desc}"):
        ...
        fig_o = px.bar(bairro_cm, x="bairro", y="tx",
                       title=f"% adimplência por bairro — {desc}",
...
fig_lx = go.Figure()
fig_lx.add_bar(x=lixo_sem["periodo"], y=lixo_sem["pago"],
               name="Arrecadado", marker_color="#1a4f82")
...
st.info(
    f"💡 **Insight Automático:** COSIP representa "
    f"{cosip_rec/(cosip_rec+alv_rec+lixo_rec+cm_rec)*100:.1f}% do total das taxas. "
    f"Gap acumulado (lançado − arrecadado): "
    f"{_fmt((alv_lanc-alv_rec)+(cosip_lanc-cosip_rec)+(lixo_lanc-lixo_rec))}. "
    f"Recomenda-se cruzar inadimplentes de alvará com o cadastro ISS."
)
